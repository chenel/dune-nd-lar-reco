...
# used inside the script we'll write out momentarily
run='000000'${SLURM_ARRAY_TASK_ID}
filename_stub=FHC.1${run:(-6)}
...
python3 -u RunChain.py --config_file $dunesoft/dune-nd-lar-reco/configs/config.inference.fullchain-pileup.yaml \
                       --model_file $data/dune/nd/nd-lar-reco/train/track+intergnn-1400evs-1000Kits-batch8/snapshot-49.ckpt \
                       --batch_size 1 \
                       --input_file $data/dune/nd/nd-lar-reco/supera/geom-20210405-pileup/${filename_stub}.larcv.root \
                       --output_file $data/dune/nd/nd-lar-reco/reco-out/geom-20210405-pileup/${filename_stub}.reco.npz \
                       --summary_hdf5 $data/dune/nd/nd-lar-reco/reco-out/geom-20210405-pileup/${filename_stub}.summary.h5
EOF
chmod +x $script
