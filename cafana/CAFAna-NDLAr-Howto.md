...
```
They correspond to the single-neutrino simulation using the ND-LAr+support structure only (no spectrometer or interactions in the detector hall walls etc.) produced by Chris Marshall in June 2021.
...
In this Howto we'll only bother with the ND CAFMaker, which was originally used to produce ntuples with truth-based "pseudo-reconstruction".
...
#### The `StandardRecord`
All CAFs contain a ROOT `TTree` which contains one custom `StandardRecord` object corresponding to each reconstructed event.
...
As the FD CAFMaker is being prepared for a move to a more structured format (see branch ), I decided to introduce the ND-LAr reco variables in a similar "structured" format.
...
There are a few known issues with the CAFs as they stand.  Please feel free to attempt fixing them if you like!
...
3. At the moment there is no integration of TMS (or ND-GAr) reco or track matching.  I am aware of effort within the TMS group to do this, but it hasn't converged yet.
