...
Dec. 2, 2021 [last updated: August 2, 2022]
Jeremy Wolcott <jwolcott@fnal.gov>

## 0. Housekeeping
* These notes have been updated since they were originally written.  For the original version, see [the GitHub page](https://github.com/chenel/dune-nd-lar-reco/blob/305cf661eaf6fca699f883b7a07323350f253a6b/tute/2021-12-02%20CAFAna-NDLAr-Howto.md).
* The original version of this Howto (see previous bullet) go with a live session with a Zoom recording.  See [the Indico page](https://indico.fnal.gov/event/52169/).  The current version of this page should still be easy enough to follow along with the recording, but you'll need to pay special attention where paths differ, if you want use the recording.
* **The CAFs described below require a number of custom software products to be set up.**  There is a lot of work in progress to make this system run more smoothly, but the CAFs that already exist live in an "in-between" world and aren't compatible with it.  Be prepared to get your hands dirty.
* The CAFs used for the demonstrations detailed below live in the shared DUNE ND Sim/Reco PNFS area:
    ```
    /dune/data/users/jwolcott/nd-lar-reco /pnfs/dune/persistent/physicsgroups/dunendsim/jwolcott/*.root
    ```
...
CAFAna has not been fully updated to handle these changes yet.
...
4. At the moment there is no integration of TMS (or ND-GAr) reco or track matching.  Some of the TMS group members and I have coordinated on getting this working, but it hasn't finalized completely yet and isn't integrated into these CAFs.
